...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private boolean isBackDeketeViewClick=false;
     ...
     */
    public SwipeListViewTouchListener(SwipeListView swipeListView, int swipeFrontView, int swipeBackView,Context context) {
    ...
    //	int secondView=swipeListView.getFirstVisiblePosition()).findViewById(swipeFrontView);
...
//                        	   }
    ...
    private void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {//可以加入判斷backView close
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
                
            }else{
            		if(actionUpX>(screenWidth-viewWidth)){
             	      isBackDeketeViewClick=true;
                  }
            }
        } else {
            if (swap) {
            	    viewWidth=200;//這邊我改寫(原本沒有這一行)
                moveTo = swapRight ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                actionUpX=(int) motionEvent.getX();
        ...
        if (doPendingDismiss) {
            animator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if(isBackDeketeViewClick){
                    	     isBackDeketeViewClick=false;
                    	     swipeListView.onItemDelete(dismissPosition);
                    }
                }
            });
        }
